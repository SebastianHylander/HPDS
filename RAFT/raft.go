...
type Raft struct {
	...
	// The highest index that has been committed
	commitIndex int
	// The highest index that has been applied to the state machine
	lastApplied int

	// Save the index of the next entry that needs to be sent to each node
	nextIndex []int
	// Save the highest index of the log that has been replicated to each node
	matchIndex []int
...
func (rf *Raft) Heartbeat(args HeartbeatArgs, reply *HeartbeatReply) error {
	...
	// If the term of the leader node is less than the current node's term
	if args.Term < rf.currentTerm {
		reply.Success = false
		reply.Term = rf.currentTerm
		return nil
	...
	// It means that the current Follower has been disconnected, so the Follower needs to inform the Leader of its current
	// maximum index, so that the Leader can return it at the next heartbeat
...
type VoteReply struct {
	// Current term number, for the candidate to update their term number
	Term int
	// True when the candidate wins this vote
	VoteGranted bool
}
...
type HeartbeatArgs struct {
	...
	PrevLogIndex int
	// Term number of PrevLogIndex
	PrevLogTerm int
	// Log entries to be stored (empty when representing heartbeat)
	Entries []LogEntry
	// Index value that the Leader has already committed
	LeaderCommit int
}
...
func (rf *Raft) broadcastHeartbeat() {
	for i := range rf.nodes {
		...
		// Extract the entry after preLogIndex - baseIndex, send to follower
...
func (rf *Raft) sendHeartbeat(serverID int, args HeartbeatArgs, reply *HeartbeatReply) {
	...
	// If the leader node is behind the follower node
	if reply.Success {
		...
		// If the leader's term is less than the follower's term, the leader needs to be converted to a follower for re-election
		if reply.Term > rf.currentTerm {
			rf.currentTerm = reply.Term
			rf.state = Follower
			rf.votedFor = -1
